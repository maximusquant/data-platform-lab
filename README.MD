...
Цель проекта — воспроизвести типичный стек команды аналитики/DWH в российских финтех- и e-commerce компаниях (Postgres/ClickHouse + dbt + Superset).
...
Датасет скачан в виде набора CSV-файлов и служит имитацией выгрузки из боевой (OLTP) системы интернет-магазина. Скачайте датасет Olist с Kaggle и поместите CSV-файлы в папку data/ по ссылке - https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce.
python ingestion/ingested_olist.py    # CSV → Postgres (landing).
python ingestion/postgres_to_clickhouse.py  # Postgres → ClickHouse (raw).
...
>  Пароли и `SUPERSET_SECRET_KEY` в `docker-compose.yml` — заглушки для локальной разработки. Если репозиторий публичный, замени `SUPERSET_SECRET_KEY` на сгенерированный (`openssl rand -base64 42`) и не используй эти же пароли где-либо ещё.
...
![LG Fact orders](docs/LG_fact_orders.png)
